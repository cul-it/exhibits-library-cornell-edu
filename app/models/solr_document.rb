...
class SolrDocument
  ...
  def update_exhibit_resource(resource_attributes)
    ...
    new_uploads_attributes = urls.each_with_object([]).with_index do |(url, new_attrs), i|
      upload_id = upload_updates[i]
      # Update existing uploads or store attrs for new uploads
      upload_id.nil? ? new_attrs << { image: url } : current_uploads.find(upload_id).update(image: url)
    end
