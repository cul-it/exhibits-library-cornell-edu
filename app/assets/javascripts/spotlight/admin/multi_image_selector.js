// Overrides multi_image_selector.js from blacklight-spotlight to set active image by tilesource instead of image id
// Uploaded images don't have an image id in the iiif manifest
//
...
(function(){
  $.fn.multiImageSelector = function(image_versions, clickCallback, activeImageId) {
    ...
    // BEGIN CUSTOMIZATION
    var imageIds = $.map(image_versions, function(e) { return e['tilesource']; });
    // END CUSTOMIZATION
    ...
    function addThumbsToList(){
      $.each(image_versions, function(i){
        // BEGIN CUSTOMIZATION
        var listItem = $('<li data-index="' + i + '"><a href="javascript:;"><img src="' + image_versions[i]['thumb'] +'" data-image-id="' + image_versions[i]['tilesource'] +'" /></a></li>');
        // END CUSTOMIZATION
